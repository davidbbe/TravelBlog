 ...
 */
function travelblog_customize_register( $wp_customize ) {
    $wp_customize->get_setting( 'blogname' )->transport         = 'postMessage';
    $wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';
    $wp_customize->get_setting( 'header_textcolor' )->transport = 'postMessage';

    // color section
    $wp_customize->add_section( 'travelblog_color_settings', array(
        'title'          => __('Main theme color','travelblog'),
        'priority'       => 35,
    ));

    $wp_customize->add_setting( 'travelblog_main_color', array(
        'default'   => '#3685cf',
        'transport' => 'postMessage',
        'sanitize_callback' => 'sanitize_hex_color',
    ));

    $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'travelblog_main_color', array(
        'label'    => __('Color Settings','travelblog'),
        'section'  => 'colors',
        'settings' => 'travelblog_main_color',
    )));


    $wp_customize->add_setting( 'travelblog_tagline_switch', array(
        'default'  => 'hide',
        'transport' => 'refresh',
        'sanitize_callback' => 'esc_attr',
    ));
    $wp_customize->add_control( 'travelblog_tagline_switcher', array(
        'settings' => 'travelblog_tagline_switch',
        'label'    => __( 'Display Tagline','travelblog' ),
    ...
    $wp_customize->add_control( 'travelblog_header_scheme', array(
        'settings' => 'travelblog_header_scheme',
        'label'    => __( 'Change header color','travelblog' ),
        'section'  => 'colors',
        'type'     => 'select',
        'choices'    => array(
            'default' => 'Default',
    ...
    if ( $wp_customize->is_preview() && !is_admin() ) {
        add_action( 'wp_footer', 'travelblog_customize_preview', 21);
    }

}
add_action( 'customize_register', 'travelblog_customize_register' );

function travelblog_customize_preview() { ?>
    <script type="text/javascript">
    ( function( $ ){
        wp.customize('travelblog_main_color',function( value ) {
    ...
    wp.customize('travelblog_tagline_switch',function( value ) {
      value.bind(function(to) {
...
function mobile_menu_css(){
    ...
    $bodytypo = ot_get_option( 'travelblog_body_font');
    $menutypo = ot_get_option( 'travelblog_menu_font');
    $logotypo = ot_get_option( 'travelblog_logo_font');
    $headerstypo = ot_get_option( 'travelblog_headers_font');
...
$custom_main_color = get_theme_mod('travelblog_main_color','#3685cf');
