...
function pp_share_btn($atts) {
    ...
    <ul>
        <li><a href="#">'.__("Share","travelblog").'</a></li>';
        ...
        if(!empty($twitter)) $output .= '<li class="share-twitter"><a target="_blank"  href="https://twitter.com/share?url=' . $url . '&amp;text=' . esc_attr($summary ). '" title="' . __( 'Twitter', 'travelblog' ) . '">Twitter</a></li>';
    $output .= '</ul>
    </div>
...
function pp_photoGrid( $atts, $content ) {
    ...
    foreach ( $attachments as $id => $attachment ) {
        ...
        $output .= ' <a href="'.$link_output[0].'" rel="grid-'.$randID.'" class="item view" ';
        if($caption == 'yes') { $output .= 'title="'. $attachment->post_excerpt; }
        $output .= '"><img src="'.$image_output[0].'" alt="">
                </a>';
    }
    $output .= "
        </div>\n";
