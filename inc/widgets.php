...
function check_sidebar_params( $params ) {
    ...
    if ( isset( $settings[ 'title' ] ) && !empty( $settings[ 'title' ] ) ) {
        ...
        $new_before_widget = str_replace("widget", "widget", $params[0][ 'before_widget' ]);
...
add_action('widgets_init', 'travelblog_load_widgets'); // Loads widgets here
function travelblog_load_widgets() {
    register_widget('travelblog_author');
    register_widget('travelblog_popular');
    if(class_exists('NS_MC_Plugin')){ register_widget('TravelBlog_NS_Widget_MailChimp'); }
}


class travelblog_author extends WP_Widget {

  private $users_split_at = 200; //Do not run get_users() if there are more than 200 users on the website
  var $defaults;

  function travelblog_author() {
    $widget_ops = array( 'classname' => 'travelblog_author', 'description' => __('Use this widget to display author/user profile info', 'travelblog') );
    $control_ops = array( 'id_base' => 'travelblog_author' );
    parent::__construct( 'travelblog_author', __('TravelBlog Author Widget', 'travelblog'), $widget_ops, $control_ops );


    $this->defaults = array(
        'title' => __('About Author', 'travelblog'),
        'author' => 0,
        'single_author' => 0,
        'display_all_posts' => 1,
        'display_email' => 1,
        'link_text' => __('View all posts', 'travelblog'),
      );

    //Allow themes or plugins to modify default parameters
    $this->defaults = apply_filters('travelblog_author_modify_defaults', $this->defaults);
  ...
  function form( $instance ) {
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php _e('Title', 'travelblog'); ?>:</label>
    ...
    <p>
      ...
      <label for="<?php echo esc_attr($this->get_field_id( 'author' )); ?>"><?php _e('Choose author/user', 'travelblog'); ?>:</label>
      ...
      <label for="<?php echo esc_attr($this->get_field_id( 'author' )); ?>"><?php _e('Enter author/user ID', 'travelblog'); ?>:</label>
    ...
    <p>
      ...
      <label for="<?php echo esc_attr($this->get_field_id( 'single_author' )); ?>"><?php _e('Automatically detect author', 'travelblog'); ?></label>
      <small class="howto"><?php _e('Use this option to show author of single post instead of pre-selected author', 'travelblog'); ?></small>
    </p>
    <h4><?php _e('Display Options', 'travelblog'); ?></h4>
    ...
    <ul>
      <li>
        ...
        <label for="<?php echo esc_attr($this->get_field_id( 'display_email' )); ?>"><?php _e('Display email of author in widget', 'travelblog'); ?></label>
      </li>
      <li>
        ...
        <label for="<?php echo esc_attr($this->get_field_id( 'display_all_posts' )); ?>"><?php _e('Display "view all posts" link', 'travelblog'); ?></label>
      </li>

      <li>
        <label for="<?php echo esc_attr($this->get_field_id( 'link_text' )); ?>"><?php _e('Link text:', 'travelblog'); ?></label>
        ...
        <small class="howto"><?php _e('Specify text for "all posts" link', 'travelblog'); ?></small>
      </li>
    </ul>
...
class travelblog_popular extends WP_Widget {

    function travelblog_popular() {
        $widget_ops = array('classname' => 'travelblog-popular', 'description' => 'Widget to display posts by selected order');
        $control_ops = array('width' => 300, 'height' => 350);
        parent::__construct('travelblog_popular', 'TravelBlog Posts', $widget_ops, $control_ops);

    $this->defaults = array(
        'title' => __('Recent Posts', 'travelblog'),
        'number' => 3
      );
    }
    ...
    function widget($args, $instance) {
        ...
        <?php echo self::showLatest( $number, $cat, $sort_by, $asc_sort_order); ?>
        </ul>
    ...
    function form($instance) {
        ...
        <br>
         <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php echo  __('Title :', 'travelblog'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
        </p>
        <p>
          <label>
            <?php _e( 'Category' ); ?>:
            <?php wp_dropdown_categories( array( 'show_option_none' => __( 'Select optional category','travelblog' ), 'name' => $this->get_field_name("cat"), 'selected' => $instance["cat"] ) ); ?>
          </label>
        </p>
        <p>
          <label for="<?php echo $this->get_field_id("sort_by"); ?>">
            <?php _e('Sort by' , 'travelblog'); ?>:
        ...
        <p>
          <label for="<?php echo $this->get_field_id("asc_sort_order"); ?>">
            <select id="<?php echo $this->get_field_id("asc_sort_order"); ?>" name="<?php echo $this->get_field_name("asc_sort_order"); ?>">
              <option value="ASC"<?php selected( $instance["asc_sort_order"], "ASC" ); ?>>ASC(ascending)</option>
              <option value="DESC"<?php selected( $instance["asc_sort_order"], "DESC" ); ?>>DESC(descending)</option>
            </select>
          </label>
        </p>
...
class TravelBlog_NS_Widget_MailChimp extends WP_Widget {
    ...
    private $travelblog_ns_mc_plugin;

    /**
     * @author James Lafferty
     * @since 0.1
     */
    public function TravelBlog_NS_Widget_MailChimp () {
        $this->default_failure_message = __('There was a problem processing your submission.','travelblog');
        $this->default_signup_text = __('Join','travelblog');
        $this->default_success_message = __('Thank you for joining our mailing list. Please check your email for a confirmation link.','travelblog');
        $this->default_title = __('Newsletter.','travelblog');
        $widget_options = array('classname' => 'widget_ns_mailchimp', 'description' => __( "Displays a sign-up form for a MailChimp mailing list.", 'travelblog'));
        parent::__construct('travelblog_ns_widget_mailchimp', __('TravelBlog MailChimp List Signup', 'travelblog'), $widget_options);
        $this->travelblog_ns_mc_plugin = NS_MC_Plugin::get_instance();
    ...
    public function form ($instance) {
        $mcapi = $this->travelblog_ns_mc_plugin->get_mcapi();
        if (false == $mcapi) {
            echo $this->travelblog_ns_mc_plugin->get_admin_notices();
        } else {
            ...
            extract($vars);
            ?>
            <h3><?php echo  __('General Settings', 'travelblog'); ?></h3>
            <p>
                <label for="<?php echo $this->get_field_id('title'); ?>"><?php echo  __('Title :', 'travelblog'); ?></label>
                <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
            </p>
            <p>
                <label for="<?php echo $this->get_field_id('desc'); ?>"><?php echo  __('Description :', 'travelblog'); ?></label>
                <textarea class="widefat" id="<?php echo $this->get_field_id('desc'); ?>" name="<?php echo $this->get_field_name('desc'); ?>"><?php echo $desc; ?></textarea>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id('current_mailing_list'); ?>"><?php echo __('Select a Mailing List :', 'travelblog'); ?></label>
                <select class="widefat" id="<?php echo $this->get_field_id('current_mailing_list');?>" name="<?php echo $this->get_field_name('current_mailing_list'); ?>">
                    ...
                    foreach ($this->lists['data'] as $key => $value) {
                        ...
                        ?>
                        <option <?php echo $selected; ?>value="<?php echo $value['id']; ?>"><?php echo __($value['name'], 'travelblog'); ?></option>
                        <?php
                    }
                    ?>
                </select>
            </p>
            <p><strong>N.B.</strong><?php echo  __('This is the list your users will be signing up for in your sidebar.', 'travelblog'); ?></p>
            <p>
                <label for="<?php echo $this->get_field_id('signup_text'); ?>"><?php echo __('Sign Up Button Text :', 'travelblog'); ?></label>
            ...
            <h3><?php echo __('Personal Information', 'travelblog'); ?></h3>
            <p><?php echo __("These fields won't (and shouldn't) be required. Should the widget form collect users' first and last names?", 'travelblog'); ?></p>
            <p>
                ...
                <label for="<?php echo $this->get_field_id('collect_first'); ?>"><?php echo  __('Collect first name.', 'travelblog'); ?></label>
                <br />
                ...
                <label><?php echo __('Collect last name.', 'travelblog'); ?></label>
            </p>
            <h3><?php echo __('Notifications', 'travelblog'); ?></h3>
            <p><?php echo  __('Use these fields to customize what your visitors see after they submit the form', 'travelblog'); ?></p>
            <p>
                <label for="<?php echo $this->get_field_id('success_message'); ?>"><?php echo __('Success :', 'travelblog'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('failure_message'); ?>"><?php echo __('Failure :', 'travelblog'); ?></label>
    ...
    public function process_submission () {
        ...
        if (isset($_GET[$this->id_base . '_email'])) {
            ...
            if (! is_email($_GET[$this->id_base . '_email'])) { //Use WordPress's built-in is_email function to validate input.
                ...
                if (false == $this->travelblog_ns_mc_plugin) {
        ...
        if ($this->successful_signup) {
            ...
            <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" id="<?php echo $this->id_base . '_form-' . $this->number; ?>" method="post">
                ...
                if ($instance['collect_first']) {
                    ?>
                    <input value="<?php echo __('First Name :', 'travelblog'); ?>" onblur="if(this.value=='')this.value='<?php echo __('First Name :', 'travelblog'); ?>';" onfocus="if(this.value=='<?php echo __('First Name :', 'travelblog'); ?>')this.value='';" type="text" name="<?php echo $this->id_base . '_first_name'; ?>" />
                    <br />
                    <br />
                    <?php
                }
                if ($instance['collect_last']) {
                    ?>
                    <input value="<?php echo __('Last Name :', 'travelblog'); ?>" onblur="if(this.value=='')this.value='<?php echo __('Last Name :', 'travelblog'); ?>';" onfocus="if(this.value=='<?php echo __('Last Name :', 'travelblog'); ?>')this.value='';" type="text" name="<?php echo $this->id_base . '_last_name'; ?>" />
                    <br />
                    <br />
                    <?php
                }
                ...
                <input class="newsletter-btn" type="submit" name="<?php echo __($instance['signup_text'], 'travelblog'); ?>" value="<?php echo __($instance['signup_text'], 'travelblog'); ?>" />
...
class WPV_Widget_Recent_Comments extends WP_Widget_Recent_Comments {
    ...
    function widget( $args, $instance ) {
        ...
        if ( isset( $cache[ $args['widget_id'] ] ) ) {
         ...
         extract($args, EXTR_SKIP);
         $output = '';
        $title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Recent Comments','travelblog' ) : $instance['title'], $instance, $this->id_base );
