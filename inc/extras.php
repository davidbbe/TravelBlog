 ...
 * @return array
 */
function travelblog_body_classes( $classes ) {
...
add_filter( 'body_class', 'travelblog_body_classes' );
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function travelblog_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'travelblog' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'travelblog_wp_title', 10, 2 );
	 ...
	 */
	function travelblog_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'travelblog_render_title' );
endif;
...
function wpv_single_parallax() {
    ...
    if(is_single()){
        ...
        wp_add_inline_style( 'travelblog-style', $custom_css );
    }
}
add_action( 'wp_enqueue_scripts', 'wpv_single_parallax' );

//customize the PageNavi HTML before it is output
add_filter( 'wp_pagenavi', 'travelblog_pagination', 10, 2 );
function travelblog_pagination($html) {
    $out = '';
...
if (!function_exists('travelblog_number_to_width')) :
function travelblog_number_to_width($width) {
    switch ($width) {
        case '1':
        return "one";
        break;
...
function wpv_share_post(){
    ...
    ?>
        ...
        <div class="share-icons-container">
            <h5 class="share-headline"><?php _e('Share','travelblog'); ?></h5><br>
            <ul class="share-icons">
                <?php if (in_array("facebook", $share_options)) { ?><li><a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo $url; ?>" title="<?php _e('Share','travelblog'); ?>"><i class="fa fa-facebook"></i></a></li> <?php } ?>
                <?php if (in_array("twitter", $share_options)) { ?><li><a href="<?php echo 'https://twitter.com/share?url=' . $url . '&amp;text=' . esc_attr($summary ); ?>" title="<?php _e('Tweet','travelblog'); ?>"><i class="fa fa-twitter"></i></a></li><?php } ?>
                <?php if (in_array("google-plus", $share_options)) { ?><li><a href="<?php echo 'https://plus.google.com/share?url=' . $url . '&amp;title="' . esc_attr($title);?>"  onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" title="<?php _e('Share','travelblog'); ?>"><i class="fa fa-google-plus"></i></a></li><?php } ?>
                <?php if (in_array("pinterest", $share_options)) { ?><li><a href="<?php echo 'http://pinterest.com/pin/create/button/?url=' . $url . '&amp;description=' . esc_attr($summary) . '&media=' . esc_attr($imageurl); ?>" title="<?php _e('Pin It','travelblog'); ?>" onclick="window.open(this.href); return false;"><i class="fa fa-pinterest-p"></i></a></li><?php } ?>
            </ul>
        </div>
...
<tr class="form-field">
<th scope="row" valign="top"><label for="map"><?php _e('Category Map','travelblog'); ?></label></th>
    <td>
...
if ( is_array( $my_posts ) && ! empty( $my_posts ) ) {
  foreach( $my_posts as $my_post ) {
    ...
    <span class="description"><?php _e('Choose what to show on category page - global map, custom map or just posts','travelblog'); ?></span>
    </td>
</tr>
<?php
}
