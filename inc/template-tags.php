...
if ( ! function_exists( 'travelblog_add_menuid' ) ) :
	function travelblog_add_menuid ($page_markup) {
	...
	add_filter('wp_page_menu', 'travelblog_add_menuid');
endif;
...
function the_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Posts navigation', 'travelblog' ); ?></h2>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( esc_html__( 'Older posts', 'travelblog' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( esc_html__( 'Newer posts', 'travelblog' ) ); ?></div>
			<?php endif; ?>
...
function the_post_navigation() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'travelblog' ); ?></h2>
		<div class="nav-links">
			<?php
...
if ( ! function_exists( 'travelblog_posted_on' ) ) :
...
if ( ! function_exists( 'travelblog_full_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function travelblog_full_posted_on() {
 	$metas = ot_get_option('pp_meta_single',array());
	...
	$posted_on = sprintf(
		esc_html_x( '%s', 'post date', 'travelblog' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = '<span class="author vcard">'.__('By','travelblog').' <a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>';
...
if ( ! function_exists( 'travelblog_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function travelblog_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' == get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'travelblog' ) );
		if ( $categories_list && travelblog_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'travelblog' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'travelblog' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'travelblog' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'travelblog' ), esc_html__( '1 Comment', 'travelblog' ), esc_html__( '% Comments', 'travelblog' ) );
		echo '</span>';
	}

	edit_post_link( esc_html__( 'Edit', 'travelblog' ), '<span class="edit-link">', '</span>' );
}
endif;
...
function the_archive_title( $before = '', $after = '' ) {
	if ( is_category() ) {
		$title = sprintf( esc_html__( 'Category: %s', 'travelblog' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		$title = sprintf( esc_html__( 'Tag: %s', 'travelblog' ), single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		$title = sprintf( esc_html__( 'Author: %s', 'travelblog' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		$title = sprintf( esc_html__( 'Year: %s', 'travelblog' ), get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'travelblog' ) ) );
	} elseif ( is_month() ) {
		$title = sprintf( esc_html__( 'Month: %s', 'travelblog' ), get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'travelblog' ) ) );
	} elseif ( is_day() ) {
		$title = sprintf( esc_html__( 'Day: %s', 'travelblog' ), get_the_date( esc_html_x( 'F j, Y', 'daily archives date format', 'travelblog' ) ) );
	} elseif ( is_tax( 'post_format' ) ) {
		if ( is_tax( 'post_format', 'post-format-aside' ) ) {
			$title = esc_html_x( 'Asides', 'post format archive title', 'travelblog' );
		} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
			$title = esc_html_x( 'Galleries', 'post format archive title', 'travelblog' );
		} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
			$title = esc_html_x( 'Images', 'post format archive title', 'travelblog' );
		} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
			$title = esc_html_x( 'Videos', 'post format archive title', 'travelblog' );
		} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
			$title = esc_html_x( 'Quotes', 'post format archive title', 'travelblog' );
		} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
			$title = esc_html_x( 'Links', 'post format archive title', 'travelblog' );
		} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
			$title = esc_html_x( 'Statuses', 'post format archive title', 'travelblog' );
		} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
			$title = esc_html_x( 'Audio', 'post format archive title', 'travelblog' );
		} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
			$title = esc_html_x( 'Chats', 'post format archive title', 'travelblog' );
		}
	} elseif ( is_post_type_archive() ) {
		$title = sprintf( esc_html__( 'Archives: %s', 'travelblog' ), post_type_archive_title( '', false ) );
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
		$title = sprintf( esc_html__( '%1$s: %2$s', 'travelblog' ), $tax->labels->singular_name, single_term_title( '', false ) );
	} else {
		$title = esc_html__( 'Archives', 'travelblog' );
	}
 ...
 * @return bool
 */
function travelblog_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'travelblog_categories' ) ) ) {
		...
		set_transient( 'travelblog_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so travelblog_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so travelblog_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in travelblog_categorized_blog.
 */
function travelblog_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'travelblog_categories' );
}
add_action( 'edit_category', 'travelblog_category_transient_flusher' );
add_action( 'save_post',     'travelblog_category_transient_flusher' );
 ...
 */
if ( ! function_exists( 'get_wpv_header_class' ) ) :
...
if ( ! function_exists( 'dimox_breadcrumbs' ) ) :
  function dimox_breadcrumbs() {
    ...
    $introtext = __('','travelblog');
    $home = __('Home','travelblog'); // text for the 'Home' link
    ...
    global $post;
    $homeLink = home_url();
    $frontpageuri = travelblog_get_posts_page('url');
    ...
    if (is_home() || is_front_page()) {
      ...
      if(function_exists('is_shop')) {
        if(is_shop()) {
          ...
          $output .= '<li><a href="'.get_permalink( $shop_page_id) .'">'.__('Shop','travelblog').'</a></li>';
        }
      }
      if ( is_category() ) {
        $thisCat = get_category(get_query_var('cat'), false);
        if ($thisCat->parent != 0) $output .= '<li>'.get_category_parents($thisCat->parent, TRUE, ' ' . $delimiter . ' ').'<li>';
        $output .= $before . __('Archive by category','travelblog').' "' . single_cat_title('', false) . '"' . $after;

      } elseif ( is_search() ) {
        $output .= $before . __('Search results for','travelblog').' "' . get_search_query() . '"' . $after;
        ...
        $output .= $before . __('Posts tagged','travelblog').' "' . single_tag_title('', false) . '"' . $after;

      } elseif ( is_author() ) {
       global $author;
       $userdata = get_userdata($author);
       $output .= $before . __('Articles posted by ','travelblog') . $userdata->display_name . $after;

     } elseif ( is_404() ) {
      $output .= $before .  __('Error 404','travelblog') . $after;
    }

    if ( get_query_var('paged') ) {
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) $output .= ' (';
        $output .= '<li>'.__('Page','travelblog') . ' ' . get_query_var('paged').'</li>';
        if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) $output .= ')';
  }
...
if ( ! function_exists( 'travelblog_get_posts_page' ) ) :

function travelblog_get_posts_page($info) {
  ...
  <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
       ...
       <div class="comment-content"><div class="arrow-comment"></div>
            <div class="comment-by"><?php printf( '<strong>%s</strong>', get_comment_author_link() ); ?>  <span class="date"> <?php printf( __( '%1$s at %2$s', 'wpv' ), get_comment_date(), get_comment_time() ); ?></span>
               <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __('<i class="fa fa-reply"></i> Reply','travelblog'), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
            </div>
            <?php comment_text(); ?>
