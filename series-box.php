...
if ( apply_filters( 'wp_post_series_enable_archive', true ) ) {
 ...
 ?>
	...
	<?php if ( is_single() && sizeof( $posts_in_series ) > 2 ) : ?>
		<a href="#" class="show-more-posts"><?php _e('Show more posts from this series','travelblog') ?></a>
	...
	<?php if ( is_single() && sizeof( $posts_in_series ) == 2 ) : ?>
		...
		<?php foreach ( $posts_in_series as $k => $next_id ) : ?>
			<a href="<?php echo esc_url(get_permalink( $next_id )); ?>" class="next-post"><?php _e('Next Post', 'travelblog') ?></a>
		<?php endforeach; ?>
		<div class="clearfix"></div>
	<?php endif; ?>
</div>